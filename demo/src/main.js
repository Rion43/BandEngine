    ...
    while (sppBuffer.length >= 2) {
        if (sppBuffer[0] !== 0xa5 || sppBuffer[1] !== 0xa5) {
            ...
            for (let i = 1; i < sppBuffer.length - 1; i++) {
                if (sppBuffer[i] === 0xa5 && sppBuffer[i + 1] === 0xa5) {
                    next = i;
                    break;
                }
            }
        ...
        if (size === null || sppBuffer.length < size)
            return;
        ...
        case SppPacketType.SESSION_CONFIG: {
            ...
            if (pkt.configOpcode === 2) {
                ...
                if (r) {
                    ...
                    if (r.version)
                        log('info', `  ✅ VERSION: ${r.version.join('.')}`);
                    if (r.maxPacketSize)
                        log('info', `  maxPacketSize: ${r.maxPacketSize}`);
                    if (r.txWin)
                        log('info', `  txWin: ${r.txWin}`);
                    if (r.sendTimeout)
                        log('info', `  sendTimeout: ${r.sendTimeout}ms`);
...
// ── Wait helpers ──
function waitOneNotification(timeoutMs) {
    return new Promise((resolve, reject) => {
        if (notifyQueue.length > 0) {
            resolve(notifyQueue.shift());
            return;
        }
        const t = setTimeout(() => { if (notifyResolve === resolve)
            notifyResolve = null; reject(new Error(`Timeout ${timeoutMs}ms`)); }, timeoutMs);
...
async function drainNotifications(initialTimeout) {
    try {
...
btnSaveKey.addEventListener('click', () => {
    if (!/^[0-9a-f]{32}$/i.test(ltkInput.value)) {
        ...
        wizardStatus.innerHTML = '❌ 32 hex karakter girin';
...
async function startConnect() {
    try {
        ...
        sppBuffer = new Uint8Array();
        notifyQueue = [];
        const device = await withTimeout(navigator.bluetooth.requestDevice({
