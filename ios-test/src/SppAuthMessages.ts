...
// Pure TS — manual varint/tag encoding, no protobufjs dependency.
// Ported from original BandEngine — no Web Crypto deps.
...
export function decodeWatchNonce(payload: Uint8Array): WatchNonceResult | null {
  ...
  while (i < wnBytes.length) {
    ...
    i += len.value;
    if (fn === 1) nonce = val;
    else if (fn === 2) hmacSig = val;
  }
  if (nonce && hmacSig) return { nonce, hmac: hmacSig };
...
export function encodeAuthDeviceInfo(
  apiLevel: number,
  phoneName: string,
  region: string,
): Uint8Array {
  ...
  const fields = [
    ...
    varintField(2, apiLevel), // phoneApiLevel
...
export function encodeAuthStep3Payload(
  encryptedNonces: Uint8Array,
  encryptedDeviceInfo: Uint8Array,
): Uint8Array {
