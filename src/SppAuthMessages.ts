...
function skipField(data: Uint8Array, i: number): number {
  ...
  switch (wireType) {
    case 0: // varint
      while (i < data.length && data[i - 1] & 0x80) i++;
      return i;
    case 1: // fixed64
      return i + 8;
...
export function decodeWatchNonce(payload: Uint8Array): WatchNonceResult | null {
  ...
  while (i < wnBytes.length) {
    ...
    const wt = tag & 0x07;
    i++;
    if (wt !== 2) { i = skipField(wnBytes, i - 1); continue; }
    let len = 0;
    let shift = 0;
    while (i < wnBytes.length) {
      const b = wnBytes[i++];
...
// Response format: Command{type=1, subtype=27, auth{status=1}}
