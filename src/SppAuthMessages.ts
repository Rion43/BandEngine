...
export function encodeCommandPhoneNonce(nonce: Uint8Array): Uint8Array {
  ...
  let off = 0;
  out.set(typeTag, off); off += typeTag.length;
  out[off++] = 1;   // COMMAND_TYPE
  out.set(subTag, off); off += subTag.length;
...
export function encodeAuthStep3Payload(encryptedNonces: Uint8Array, encryptedDeviceInfo: Uint8Array): Uint8Array {
  ...
  let off = 0;
  out.set(f1Tag, off); off += f1Tag.length;
  out[off++] = encryptedNonces.length;
  out.set(encryptedNonces, off); off += encryptedNonces.length;
  out.set(f2Tag, off); off += f2Tag.length;
...
export function encodeCommandAuthStep3(authStep3: Uint8Array): Uint8Array {
  ...
  let off = 0;
  out.set(tTag, off); off += tTag.length;
  out[off++] = 1;
  out.set(sTag, off); off += sTag.length;
  out[off++] = 27;
  out.set(cmdAuth, off);
  return out;
}
